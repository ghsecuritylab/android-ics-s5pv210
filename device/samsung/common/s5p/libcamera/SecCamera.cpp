...
/*
 *
 *Interface file between Camera HAL and V4L2/FIMC Driver.
 *
 */
...
SecCamera::SecCamera() :
            ...
            m_recording_en        (0),
            m_recording_width     (720),
            m_recording_height    (480),
            m_angle(-1),
            m_anti_banding(-1),
            m_wdr(-1),
            m_anti_shake(-1),
...
int SecCamera::setVerticalMirror(int cam_fd)
{
    ALOGV("%s :", __func__);
    ...
    if (fimc_v4l2_s_ctrl(cam_fd, V4L2_CID_VFLIP, 1) < 0) {
...
int SecCamera::setHorizontalMirror(int cam_fd)
{
    ALOGV("%s :", __func__);
    ...
    if (fimc_v4l2_s_ctrl(cam_fd, V4L2_CID_HFLIP, 1) < 0) {
...
void SecCamera::setExifChangedAttribute()
{
    ...
    //2 0th IFD Exif Private Tags
    //3 Exposure Time
//    int shutterSpeed = fimc_v4l2_g_ctrl(m_cam_fd,  V4L2_CID_CAMERA_GET_SHT_TIME);
