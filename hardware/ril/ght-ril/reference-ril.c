...
static void requestRadioPower(void *data, size_t datalen, RIL_Token t)
{
    ...
    if (onOff == 0 && sState != RADIO_STATE_OFF) {
		LOGE("fantao: radio off cannceled\n");
...
static void requestOrSendDataCallList(RIL_Token *t)
{
    ...
    RIL_Data_Call_Response_v4 *responses =
        alloca(n * sizeof(RIL_Data_Call_Response_v4));
    ...
    RIL_Data_Call_Response_v4 *response = responses;
    for (p_cur = p_response->p_intermediates; p_cur != NULL;
         p_cur = p_cur->p_next) {
    ...
    if (t != NULL)
        RIL_onRequestComplete(*t, RIL_E_SUCCESS, responses,
                              n * sizeof(RIL_Data_Call_Response_v4));
    else
        RIL_onUnsolicitedResponse(RIL_UNSOL_DATA_CALL_LIST_CHANGED,
                                  responses,
                                  n * sizeof(RIL_Data_Call_Response_v4));
...
static void selectToneChannel(void *param)
{
	at_send_command("AT+MAPATH=2,3,2", NULL);
}
static void selectSpeechChannel(void *param)
{  
	at_send_command("AT+MAPATH=2,3,1", NULL);
}
static  needSelectChanel= 1;
static void requestGetCurrentCalls(void *data, size_t datalen, RIL_Token t)
{
    ...
    for (countValidCalls = 0, p_cur = p_response->p_intermediates
	  ...
	  if(needSelectChanel)
            RIL_requestTimedCallback (selectToneChannel, NULL, NULL);
	    needSelectChanel = 0;
            needRepoll = 1;
        }else if(p_calls[countValidCalls].state == RIL_CALL_ACTIVE){
        	needSelectChanel = 1;
		RIL_requestTimedCallback (selectSpeechChannel, NULL, NULL);
	}
        countValidCalls++;
    }
...
static void requestNetworkList(void *data, size_t datalen, RIL_Token t)
{
    ...
    err = at_send_command_singleline_timeout("AT+COPS=?", "+COPS:", &p_response, 30000);
...
static int init_pppd_script_file(char *data_port, char *user, char *password, char *connect_script)
{
	...
	size_t size, len = 0;
	ssize_t ret_len;
	char buf[buf_len + 1], buf_file[buf_len + 1];
	int fd;

	if (!data_port || !connect_script) {
		return -1;
	}

	size = strlen(data_port);
	if (len + size + 1 > buf_len) {
		return -1;
	}
	strcpy(&buf[len], data_port);
    len += size;
	strcpy(&buf[len], "\n");
	len += 1;
	...
	strcpy(&buf[len], param);
	len += size;

	if (user && user[0]) {
		size = strlen(user);
		if (len + size + 6 + 2 > buf_len) {
			return -1;
		}
		strcpy(&buf[len], "user \"");
        len += 6;
	...
	size = strlen(connect_script);
	if (len + size + 35 + 2 > buf_len) {
		return -1;
	}
	
	strcpy(&buf[len], "connect \"/system/bin/chat -s -v -f ");
    len += 35;
	strcpy(&buf[len], connect_script);
    len += size;
	strcpy(&buf[len], "\"\n");
	len += 2;
	LOGD("len = %d ", len);
	...
	fd = open("/data/misc/pppd/pppd-scripts", O_RDONLY);
	if (fd < 0) {
		goto replace_script;
	}
	ret_len = read(fd, buf_file, buf_len);
	if (ret_len < 0 || ret_len >= (ssize_t)buf_len) {
		close(fd);
		goto replace_script;
	}
	if (ret_len != (ssize_t)len) {
		close(fd);
		goto replace_script;
	}
	if (memcmp(buf, buf_file, len) == 0) {
		close(fd);
		goto replace_script_done;
	}
	close(fd);

replace_script:
	LOGD("replace_script");
	fd = open("/data/misc/pppd/pppd-scripts", O_CREAT|O_WRONLY|O_TRUNC, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH);
	if (fd == -1) {
		LOGD("open /data/misc/pppd/pppd-scripts failed");
		return -1;
	}
	ret_len = write(fd, buf, len);
	if (ret_len != (ssize_t)len) {
		close(fd);
		LOGD("write /data/misc/pppd/pppd-scripts");
		return -1;
	}
	close(fd);
replace_script_done:
...
#define PPPD_LOCAL_IP "net.ppp0.local-ip"

static pid_t mPid = 0;
static void requestSetupDataCall(void *data, size_t datalen, RIL_Token t)
{
    const char *apn, *user, *password;
    int err;
	pid_t pid;
    int retries;
    int max_retries = 6;
	
    char *response[3] = { "1", (char*)"ppp0", (char*)"0.0.0.0" };
    char local_ip[PROPERTY_VALUE_MAX];
    char pppd_status[PROPERTY_VALUE_MAX];
    char pppd_data_port[PROPERTY_VALUE_MAX];
    int pppd_started = 0;
	struct ifreq ifr;
    ...
    err = property_get("net.ppp0.status", pppd_status, "");
 	if (err > 0 && strcmp(pppd_status, "running") == 0) {
        LOGE("====> warning : pppd service state is %s\n", pppd_status);
        goto error;
    }
	if(mPid)
	{
		LOGE("pppd service state is on, mPid = %d", mPid);
		goto error;
	}
	if(strcmp(apn, "cmnet") == 0) {
        err = init_pppd_script_file("/dev/mux1", NULL, NULL, "/system/etc/ppp/cmnet-gprs");
    } 
	else if(strcmp(apn, "cmwap") == 0){
    	err = init_pppd_script_file("/dev/mux1", NULL, NULL, "/system/etc/ppp/cmwap-gprs");
	}
	else if(strcmp(apn, "uninet") == 0 || strcmp(apn, "3gnet") == 0){
    	err = init_pppd_script_file("/dev/mux1", NULL, NULL, "/system/etc/ppp/uninet-gprs");
    }
	else if(strcmp(apn, "uniwap") == 0 || strcmp(apn, "3gwap") == 0){
    	err = init_pppd_script_file("/dev/mux1", NULL, NULL, "/system/etc/ppp/uniwap-gprs");
    }
    ...
    property_set(PPPD_LOCAL_IP, "");

	 if ((pid = fork()) < 0) {
        LOGE("Data Call fork failed (%s)", strerror(errno));
        goto error;
    }
    if (!pid) {
        if (execl("/system/bin/pppd", "/system/bin/pppd", "file", "/data/misc/pppd/pppd-scripts", (char *) NULL)) 
        {
            LOGE("execl failed (%s)", strerror(errno));
        }
        LOGE("Data Call Should never get here!");
        return;
    } else {
        mPid = pid;
    }
    ...
    for (retries = 0; retries < max_retries; retries++) {
        ...
        if (s >= 0) {
            ...
            strcpy((char *)&(ifr.ifr_name), "ppp0");
            if (ioctl(s, SIOCGIFFLAGS, &ifr) == 0) {
                if (ifr.ifr_flags & IFF_UP) {
                    LOGD("====> info : pppd interface is up\n");
                    close(s);
                    sleep(2);
                    break;
                }
	...
	if (retries >= max_retries) {
        LOGE("====> warning : start pppd service timeout\n");
        goto error;
    }
   
	err = property_get("net.ppp0.status", pppd_status, "");
 	if (err > 0 && strcmp(pppd_status, "running") != 0) {//!max_retries && 
        LOGE("====> warning : pppd service state is %s\n", pppd_status);
        goto error;
    }

    err = property_get(PPPD_LOCAL_IP, local_ip, "");
    if (err < 0 || strcmp(local_ip, "") == 0) {
        LOGE("====> warning : pppd service no local-ip\n");
        goto error;
    }
...
static void requestDeactivateDataCall(void *data, size_t datalen, RIL_Token t)
{
    ...
    LOGD("====> requesting deactivating data connection with CID '%s'\n", cid);
	
	if (mPid == 0) {
        LOGE("====> warning : pppd service have not been started\n");
		goto  _return;
	}
	err = property_get("net.ppp0.status", pppd_status, "");
 	if (err > 0 && strcmp(pppd_status, "running") != 0) {
	...
	if(0 == waitpid( mPid, NULL, WNOHANG))
	{
		if(!kill(mPid, SIGTERM))
		{
	...
	for (retries = 0; retries < 20; retries++) {
		err = property_get("net.ppp0.status", pppd_status, "");
		LOGD("====> warning : pppd service state is %s\n", pppd_status);
		if (err <= 0 || strcmp(pppd_status, "stopped") == 0) {
			break;
		}
		sleep(2);
	}
...
static void requestGetIMSI(void *data, size_t datalen, RIL_Token t)
{
	...
	if(at_tok_nextstr(&line,&imsi) <0 ) goto error;
...
static void requestGetIMEI(void *data, size_t datalen, RIL_Token t)
{
	...
	if(at_tok_nextstr(&line,&imei) <0 ) goto error;
...
void requestBasebandVersion(void *data, size_t datalen, RIL_Token t)
{
    ...
    line = p_response->p_intermediates->line;
    if (at_tok_start(&line) < 0)  goto error;
    if (at_tok_nextstr(&line, &ver) < 0) goto error;
...
static void  requestIMEISV(void * data, size_t datalen, RIL_Token t)
{
	 char * sv = "01";
...
static void requestDTMF(void * data, size_t datalen, RIL_Token t)
{
    ...
    asprintf(&cmd,"AT+VTS=%c,%d", (int)c, (int)1);
...
{
    int err;
    /* Send a command to cancel teh DTMF tone*/
...
static void initializeCallback(void *param)
{
    ...
    at_handshake();//wavelet 2012-10-31
   ...
   //at_send_command("ATE0Q0V1", NULL);
    ...
    /*  Extended errors */
    at_send_command("AT+CMEE=2", NULL);
    ...
    at_response_free(p_response);
 	...
 	//at_send_command("AT+XREG=1", NULL);//add by wanz
	...
	at_send_command("ATS24=10", NULL);
	at_send_command("ATS96=3", NULL);
	at_send_command("ATS94=3", NULL);
	//at_send_command("AT+CLVL=6", NULL);
	at_send_command("AT+MMICG=13", NULL);
	at_send_command("AT+MAVOL=3,7,3", NULL);
	//at_send_command("AT+MAVOL=3,5,7", NULL);
	at_send_command("AT+MMICV=1", NULL);
	at_send_command("AT+MAPATH=1,2,1", NULL);
	at_send_command("AT+MAPATH=2,3,1", NULL);
	at_send_command("AT+MAPATH=2", NULL);
	//-----
	at_send_command("AT+CALM=1,1", NULL);
	at_send_command("AT+CRSL=7", NULL);
	//at_send_command("AT+CRTT=1,1", NULL);
	//-----
	 ...
	 property_set("ctl.start", "gsmmux");
        ...
        while  (fd < 0) {
			...
			else if (s_device_path != NULL) 
			{
                ...
                //if ( fd >= 0 && !memcmp( s_device_path, "/dev/ttyA", 9 ) ) {//USB  and serial
			if (fd > 0)
		     	{
     				LOGD("+++open %s sucess", s_device_path);    
				#if 0
            ...
            if (fd < 0) {
                LOGE ("opening AT interface: %s, retrying...", s_device_path);
                sleep(10);
                /* never returns */
            }
        }
