...
{
#define RESEND_COUNT 3   //ADD by trento for count resend command if timeout
	int count = 0;
    int err = 0;
#ifndef USE_NP
    struct timespec ts;
...
retry:
    ...
    while (sp_response->finalResponse == NULL && s_readerClosed == 0) {
        if (timeoutMsec != 0) {
			...
			if(!smspdu)
				setTimespecRelative(&ts, 1000);
			else
		    {
				setTimespecRelative(&ts, 5000);
			}
            err = pthread_cond_timedwait(&s_commandcond, &s_commandmutex, &ts);
        }

        if (err == ETIMEDOUT) {
		/*resend command after timeout by trento 0529*/
    ...
    else if(0 == sp_response->success)
    {
       err = at_get_cme_error_2(sp_response);
                    ...
                    long long timeoutMsec, ATResponse **pp_outResponse)
{
    ...
    if(smspdu)
    {
		for(;err== AT_ERROR_TIMEOUT;)
		{
			err=at_send_command_full_nolock(command, type, responsePrefix, smspdu, timeoutMsec, pp_outResponse);
	    }
    }
    pthread_mutex_unlock(&s_commandmutex);
...
AT_CME_Error at_get_cme_error_2(const ATResponse *p_response)
{
    int ret;
    int err;
    char *p_cur;
